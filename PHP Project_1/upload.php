...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_FILES["avatar"])) {
        ...
        if (count($errors) > 0) {
           ...
           $time=time();
           $hashedTime=md5($time);
           $newPath="avatar/$hashedTime.$ext";
           $isUploaded=move_uploaded_file($tmpFile,$newPath);
           if($isUploaded){
            $_SESSION["success"] = "Avatar is uploded success";
            $query="INSERT INTO avatar(name,extension,size) VALUES(:n,:e,:s)";
            ...
            $stmt->execute([
              ...
              ":s"=>$size,
               ...
               array_push($errors,"something worng");
               $_SESSION["errors"] = $errors;
           }
            header("Location:upload.php");
            return;
